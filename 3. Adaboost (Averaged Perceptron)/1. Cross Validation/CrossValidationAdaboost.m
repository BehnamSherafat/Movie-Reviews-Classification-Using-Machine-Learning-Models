function [xForDisplay] = CrossValidationAdaboost()
...
for i = 1:cross_fold    
    for m = 1:size(learning_rate, 2)    
        ...
        for l = 1:T            
            ...
            for k=1:testingDataSize
                DExpalphaYH(k, 1) = D(k, 1)*exp(-alpha(l, 1)*test(k, 1)*Classifications(k, 2));
            end
        ...
        for k=1:testingDataSize
                ...
                summ = 0;
                for n = 1:T
                    summ = summ + functions{n,2}*sign(dot([test(k, 2:num_features+1), 1], functions{n,3}));
                end
                if sign(summ) >= 0
                    Classifications(k, 2) = +1;
                else
